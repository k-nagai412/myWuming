...
contains
  ...
  subroutine init()
    ...
    !v0   = u0/gam0
    wpe  = sqrt(betai*rtemp)*c/(sqrt(2.0d0)*alpha*ldb)
    wge  = wpe / alpha
    wpi  = wpe / sqrt(mass_ratio)
    wgi  = wge / mass_ratio
    va   = c /(alpha*sqrt(mass_ratio)) !
    v0   = -abs(va * ma)   !
    gam0 = 1./sqrt(1.-(v0/c)**2) !
    u0   = v0 * gam0 !
    rge  = alpha * ldb * sqrt(2.0d0)   !
    rgi  = rge *sqrt(mass_ratio) /sqrt(rtemp) !
    vte  = rge * wge !
    vti  = vte / (sqrt(mass_ratio)*sqrt(rtemp)) !
    r(1) = mass_ratio !ion
  ...
  subroutine save_param(n0, wpe, wpi, wge, wgi, vti, vte, filename)
    ...
    real(8), intent(in)          :: wpe, wpi, wge, wgi, vte, vti
